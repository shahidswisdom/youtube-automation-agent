...
class DailyAutomation {
  ...
  async getDueTasks() {
    ...
    const wasRunToday = async (key) => {
      const lastRun = await this.db.getSetting(key);
      return lastRun ? lastRun.startsWith(today) : false;
    };

    if (currentHour >= 6 && !(await wasRunToday('last_content_generation'))) {
      dueTasks.push({ name: 'daily-content-generation', method: 'runDailyContentGeneration' });
    }

    const lastPublishCheck = await this.db.getSetting('last_publish_check');
    if (!lastPublishCheck || (now - new Date(lastPublishCheck)) > 30 * 60 * 1000) {
      dueTasks.push({ name: 'publish-queue-processing', method: 'processPublishQueue' });
    }

    if (currentHour >= 9 && !(await wasRunToday('last_analytics_collection'))) {
      dueTasks.push({ name: 'daily-analytics', method: 'collectDailyAnalytics' });
    }

    if (currentDay === 0 && currentHour >= 8 && !(await wasRunToday('last_strategy_review'))) {
      dueTasks.push({ name: 'weekly-strategy-review', method: 'weeklyStrategyReview' });
    }

    if (currentHour >= 22 && !(await wasRunToday('last_daily_optimization'))) {
      dueTasks.push({ name: 'daily-optimization', method: 'runDailyOptimization' });
    }

    if (currentDay === 6 && currentHour >= 3 && !(await wasRunToday('last_db_maintenance'))) {
      dueTasks.push({ name: 'database-maintenance', method: 'databaseMaintenance' });
    }

    return dueTasks;
  }

  async runDueTask(task) {
    const { name, method } = task;
    this.logger.info(`Running cloud task: ${name}`);

    try {
      await this[method]();

      const settingKey = {
        'daily-content-generation': 'last_content_generation',
        'publish-queue-processing': 'last_publish_check',
        'daily-analytics': 'last_analytics_collection',
        'weekly-strategy-review': 'last_strategy_review',
        'daily-optimization': 'last_daily_optimization',
        'database-maintenance': 'last_db_maintenance'
      }[name];
      ...
      this.logger.success(`Cloud task completed: ${name}`);
      return true;
    } catch (error) {
      this.logger.error(`Cloud task failed: ${name}`, error);
      return false;
    }
  }
